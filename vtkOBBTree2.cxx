...
vtkStandardNewMacro(vtkOBBTree2);
...
void vtkOBBTree2::FreeSearchStructure()
{
  if ( this->Tree )
    {
...
void vtkOBBTree2::DeleteTree(vtkOBBNode *OBBptr)
{
  if ( OBBptr->Kids != NULL )
    {
...
void vtkOBBTree2::ComputeOBB(vtkPoints *pts, double corner[3], double max[3],
...
void vtkOBBTree2::ComputeOBB(vtkDataSet *input, double corner[3], double max[3],
                            double mid[3], double min[3], double size[3])
{
...
// and the three axes defining the orientation of the OBB. Also return
// a sorted list of relative "sizes" of axes for comparison purposes.
void vtkOBBTree2::ComputeOBB(vtkIdList *cells, double corner[3], double max[3],
                            double mid[3], double min[3], double size[3])
{
...
static inline
int vtkOBBTree2LineIntersectsTriangle(double p1[3], double p2[3],
...
int vtkOBBTree2::InsideOrOutside(const double point[3])
{ 
  // no points!  
...
int vtkOBBTree2::IntersectWithLine(const double p1[3], const double p2[3],
                                  vtkPoints *points, vtkIdList *cellIds)
{
      ...
      if (node->Kids == NULL)
        ...
        for (vtkIdType i = 0; i < numCells; i++)
          ...
          for (vtkIdType j = 0; j < numPts-2; j++)
            ...
            if (vtkOBBTree2LineIntersectsTriangle((double *)p1, (double *)p2,
                                                 pt1, pt2, pt3,
  ...
  if (listSize != 0)
    ...
    {
    	double point[3];
     	point[0] = p1[0] + distanceList[j]*v12[0];
      point[1] = p1[1] + distanceList[j]*v12[1];
      point[2] = p1[2] + distanceList[j]*v12[2];
      points->InsertNextPoint(point);
    	cellIds->InsertNextId(cellList[j]);	   
    }
   }
...
// finite line
int vtkOBBTree2::IntersectWithLine(double a0[3], double a1[3], double tol,
...
void vtkOBBTree2::BuildTree(vtkIdList *cells, vtkOBBNode *OBBptr, int level)
{
...
void vtkOBBTree2::GenerateRepresentation(int level, vtkPolyData *pd)
{
  vtkPoints *pts;
  vtkCellArray *polys;
...
void vtkOBBTree2::GeneratePolygons(vtkOBBNode *OBBptr, int level, int repLevel,
...
int vtkOBBTree2::DisjointOBBNodes( vtkOBBNode *nodeA,
                                  vtkOBBNode *nodeB,
...
int vtkOBBTree2::TriangleIntersectsNode( vtkOBBNode *nodeA,
...
int vtkOBBTree2::LineIntersectsNode( vtkOBBNode *pA,
...
int vtkOBBTree2::IntersectWithOBBTree( vtkOBBTree2 *OBBTreeB,
                                      vtkMatrix4x4 *XformBtoA,
...
void vtkOBBTree2::PrintSelf(ostream& os, vtkIndent indent)
{
